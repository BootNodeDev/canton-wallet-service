...
This service aligns its wire format with this spec but does not depend on the
upstream codegen toolchain. The TS types in `src/types.ts` are hand-authored
against the schemas in this file.
...
gh api repos/canton-network/wallet/contents/api-specs/openrpc-dapp-api.json --jq '.content' \
  | base64 -d > api-specs/openrpc-dapp-api.json
```
